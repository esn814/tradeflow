...
router.get('/', (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[settings] Fetch failed'); res.status(500).json({ error: 'Failed to fetch settings' }); }
});
...
router.put('/', (req, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, '[settings] Update failed'); res.status(500).json({ error: 'Failed to update settings' }); }
});
...
router.get('/demo-trades', (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[settings] Fetch demo trades failed'); res.status(500).json({ error: 'Failed to fetch demo trades' }); }
});
...
router.post('/demo-trades', (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[settings] Record demo trade failed'); res.status(500).json({ error: 'Failed to record demo trade' }); }
});
