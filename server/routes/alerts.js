...
router.get('/', (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[alerts] Fetch failed'); res.status(500).json({ error: 'Failed to fetch alerts' }); }
});
...
router.post('/', validateBody(createAlertSchema), (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[alerts] Create failed'); res.status(500).json({ error: 'Failed to create alert' }); }
});
...
router.put('/:id', validateBody(updateAlertSchema), (req, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, '[alerts] Update failed'); res.status(500).json({ error: 'Failed to update alert' }); }
});
...
router.delete('/:id', (req, res) => {
  try {
    ...
    if (result.changes === 0) return res.status(404).json({ error: 'Alert not found' });
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, '[alerts] Delete failed'); res.status(500).json({ error: 'Failed to delete alert' }); }
});
