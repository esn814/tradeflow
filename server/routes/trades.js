...
router.get('/', (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[trades] Fetch failed'); res.status(500).json({ error: 'Failed to fetch trades' }); }
});
...
router.get('/summary', (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[trades] Summary failed'); res.status(500).json({ error: 'Failed to compute summary' }); }
});
...
router.post('/', validateBody(createTradeSchema), (req, res) => {
  ...
  } catch (err) { logger.error({ err }, '[trades] Record failed'); res.status(500).json({ error: 'Failed to record trade' }); }
});
