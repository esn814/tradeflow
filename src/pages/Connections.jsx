  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Stat label="Wallets Connected" value={`${connectedWallets.length}`} color={connectedWallets.length > 0 ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-muted)]'} sub={connectedWallets.length > 0 ? connectedWallets.map(w => w.name).join(', ') : 'None'} subClassName="truncate" />
